...
import 'package:demo_app/common/bloc/pump_station_history/pump_station_history_state_cubit.dart';
...
import '../domain/usecases/pump_station_history.dart';

class PumpStationHistoryContent extends StatefulWidget {
  final int userId;
  final String userRole;

  const PumpStationHistoryContent({
    Key? key,
  ...
  @override
  State<PumpStationHistoryContent> createState() =>
      _PumpStationHistoryContentState();
}

class _PumpStationHistoryContentState extends State<PumpStationHistoryContent>
  ...
  bool get _isAdmin {
    return widget.userRole == 'ADMIN' || widget.userRole == 'SUPER_ADMIN';
  }
  ...
  void _loadHistory(BuildContext ctx) {
    ctx.read<PumpStationHistoryCubit>().loadPumpStationHistory(
      useCase: serviceLocator<PumpStationHistoryUseCase>(),
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => PumpStationHistoryCubit()
          ...
          return Column(
            children: [
              ...
              Expanded(
                child:
                    BlocBuilder<
                      PumpStationHistoryCubit,
                      PumpStationHistoryState
                    >(
                      builder: (context, state) {
                        if (state is PumpStationHistoryLoadingState) {
                          return const _CenteredLoader();
                        }

                        if (state is PumpStationHistoryErrorState) {
                          return _ErrorView(
                        ...
                        if (state is PumpStationHistoryLoadedState) {
...
// [Continue with all the helper widgets: _BrandButton, _GhostButton,
//  _CenteredLoader, _EmptyView, _ErrorView, _InfoTile, _TimeRow,
//  _PaginationBar, _DatePickerField, _AnimatedIn - exactly as in original]

// ... (I'll add the complete helper widgets in the next part due to length)
