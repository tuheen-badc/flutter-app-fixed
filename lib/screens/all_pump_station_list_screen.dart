...
class AllPumpStationControlScreen extends StatefulWidget {
  final User userData;

  const AllPumpStationControlScreen({Key? key, required this.userData})
    : super(key: key);

  @override
  State<AllPumpStationControlScreen> createState() =>
      _AllPumpStationControlScreenState();
}

class _AllPumpStationControlScreenState
    extends State<AllPumpStationControlScreen> {
  ...
  void _loadStations() {
    ...
    _providerContext!.read<AllPumpStationCubit>().loadPumpStations(
      useCase: serviceLocator<AllPumpStationListUseCase>(),
      params: criteria,
    );
  }
  ...
  void _togglePump(BuildContext context, PumpStationItem station) {
    context.read<AllPumpControlButtonCubit>().togglePumpStation(
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (_) => AllPumpStationCubit()
            ..loadPumpStations(
              useCase: serviceLocator<AllPumpStationListUseCase>(),
              params: PumpStationCriteria(page: currentPage, size: pageSize),
            ),
        ),
        BlocProvider(create: (_) => AllPumpControlButtonCubit()),
      ],
      child: Builder(
        builder: (builderContext) {
          ...
          return MultiBlocListener(
            listeners: [
              BlocListener<
                AllPumpControlButtonCubit,
                AllPumpControlButtonState
              >(
                listener: (context, state) {
                  if (state is AllPumpControlButtonSuccessState) {
                    ...
                    Future.delayed(const Duration(milliseconds: 500), () {
                      if (mounted) {
                        context.read<AllPumpControlButtonCubit>().resetState();
                      }
                    });

                    // Reload stations to get backend confirmation (if available)
                    _loadStations();
                  } else if (state is AllPumpControlButtonFailureState) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                  ...
                  Expanded(
                    child: BlocBuilder<AllPumpStationCubit, AllPumpStationState>(
                      builder: (context, state) {
                        if (state is AllPumpStationLoadingState) {
                          return const _CenteredLoader();
                        }

                        if (state is AllPumpStationErrorState) {
                          return _ErrorView(
                        ...
                        if (state is AllPumpStationLoadedState) {
...
// Request info model for local state
class RequestInfo {
    ...
    return ElevatedButton(
      ...
      style:
          ElevatedButton.styleFrom(
            backgroundColor: _AllPumpStationControlScreenState._brand,
    ...
    return TextButton(
      ...
      style:
          TextButton.styleFrom(
            foregroundColor: _AllPumpStationControlScreenState._textSecondary,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: _AllPumpStationControlScreenState._border),
      ),
      child: Row(
        children: [
          Icon(
            icon,
            size: 16,
            color: _AllPumpStationControlScreenState._textSecondary,
          ),
          const SizedBox(width: 8),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: const TextStyle(
                    fontSize: 12,
                    fontWeight: FontWeight.w700,
                    color: _AllPumpStationControlScreenState._textPrimary,
                  ),
...
class _ControlButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<AllPumpControlButtonCubit, AllPumpControlButtonState>(
      builder: (context, buttonState) {
        final isLoading =
            buttonState is AllPumpControlButtonLoadingState &&
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: station.running
                  ? _AllPumpStationControlScreenState._danger
                  : _AllPumpStationControlScreenState._success,
              foregroundColor: Colors.white,
              ...
              elevation: 0,
              disabledBackgroundColor: _AllPumpStationControlScreenState._muted,
            ),
            child: isLoading
                ? const SizedBox(
                    height: 20,
  ...
  Widget build(BuildContext context) {
    return const Center(
      child: SizedBox(
        ...
        height: 44,
        child: CircularProgressIndicator(
          color: _AllPumpStationControlScreenState._brand,
          strokeWidth: 3,
        ),
      ),
    );
...
class _PaginationBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: _AllPumpStationControlScreenState._surface,
        border: const Border(
          top: BorderSide(color: _AllPumpStationControlScreenState._border),
        ),
        boxShadow: [
          BoxShadow(
